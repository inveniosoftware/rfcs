...
- updating the role of the requesting member
...
- As a user (e.g., new lab member), I want to be able to request to join the communities I am interested in
...
### Review membership requests (review flow)
...
If declined, the initial "Request Membership" button will show again. This follows from implementation - the Member entry in the database will have been deleted in case of decline. Although history of the interaction is kept in the storage associated with the `ArchivedMemberRequest` (it is the result of renaming `ArchivedInvitation` -> `ArchivedMemberRequest`) `api.py` class. This is in keeping with what is done for Invitations.

### Expiry of membership requests

Membership requests can expire like most other requests. The request's lifespan can be controlled by the configuration:
...
where the default is set to 30 days.

### Aftermath of membership request process

All membership requests that concluded their lifecycle (accepted, cancelled, declined, or expired) will be present in the storage associated with the `ArchivedMemberRequest`. They are visible via the "Closed" toggle of the Membership Requests section of "Members" tab in the community dashboard.

Changing the role of a closed membership request's member is not possible.

#### Note on JSON API links

"MemberRequests" (Invitations and Membership Requests) need to provide the action links of the request related to them. However, these classes are not requests themselves and do not duplicate storage of a full request's data. As such, usual methods for generating these links cannot be used and these links are provided in a custom fashion. In particular, the
actions are explicitly embedded rather than dynamically retrieved in those scenarios. Knowledge of search permissions are also used to assume and simplify permission checking.

## Additional resources

This issue contains much of the original mockups and discussions: https://github.com/inveniosoftware/invenio-communities/issues/855 .
...
## Out of scope / further enhancements in the future
- Make the "Request Membership" modal overridable.
- This flow is only for parent communities. Sub-communities are not considered.
- Notification pills/markers

## Migration strategy

The following commands to update some mappings need to be executed (whether feature is used or not) as the work entails the addition of the request type in the Member storage.
