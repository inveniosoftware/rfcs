...
If declined, the initial "Request Membership" button will show again. This follows from implementation - the Member entry in the database will have been deleted in case of decline. Although history of the interaction is kept in the storage associated with the renamed `ArchivedMemberRequest` (`ArchivedInvitation` -> `ArchivedMemberRequest`) api.py class. This is in keeping with what is done for Invitations.

#### Note on JSON API links

"MemberRequests" (Invitations and Membership Requests) need to provide the action links of the request related to them. However, these classes are not requests themselves and do not duplicate storage of a full request's data. As such, some shortcuts are used to provide these links. In particular, the
actions are explicitly embedded rather than dynamically retrieved in those scenarios. Knowledge of search permissions are also used to assume part of the permissions. 
